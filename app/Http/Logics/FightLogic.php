...
class FightLogic extends Logic
{
    /**
     * 战斗主函数
     *
    ...
    private function checkAlive($my_id)
    {
     ...
     * @author Zhou Yu
     */
    private function booty(int $result, Model $my, Model $enemy)
    {
        /**
     ...
     * @author Zhou Yu
     */
    private function calc(Model $my, Model $enemy)
    {
        return $my->power <=> $enemy->power;
    }

    private function calcBody($result_int, $my_id)
    {
        $ctl = new FleetBodyController();
        $bodies = $ctl->loc()->index($my_id);
     ...
     * @param $enemy_id
     */
    private function calcContribution($result_int, $my_id, $enemy_id)
    {
        switch ($result_int) {
            case 1:
